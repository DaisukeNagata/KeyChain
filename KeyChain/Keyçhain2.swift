...
public class TateruKeyChainService: NSObject {
    ...
    //認証Tokenを保存するKey
    public class func savePassword(token: String) { self.save(service: passwordkey , data: token) }

    //メールアドレスを保存するKey
    public class func mailAddress(token:String) { self.save(service: mailKey, data: token) }

    //アドレスを保存するKey
    public class func userKey(token:String) { self.save(service: addressKey, data: token) }

    //認証Tokenをロードするメソッド
    public class func loadToken() -> String? { return self.load(service: passwordkey) }

    //メールをロードするKey
    public class func loadMailKey() -> String? { return self.load(service: mailKey) }

    //アドレスをロードするKey
    public class func loadUserKey() -> String? { return self.load(service: addressKey) }

    //認証Tokenの削除
    public class func passwordRemove(token:String) -> String? { return self.remove(service: token) }
    
    //メールの削除
    public class func mailKeyRemove(token:String) -> String? { return self.remove(service: token) }
    
    //アドレスの削除
    public class func addressRemove(token:String)->String?{ return self.remove(service: token) }
